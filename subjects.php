...
<?php
 ...
 if(isset($_POST['reg'])or isset($_POST['semi']))
 {
  ...
  // $student_reg_no=$id_no;
   ...
   $result1=mysql_query("select subject_code,subject_name from programstructure where regulation_code='$regulation_code' and semister='$sem'")or die("failed to query database".mysql_error());
	...
	while($row=mysql_fetch_array($result1,MYSQL_ASSOC)){
		 ...
		 $i=$row['subject_code'];
       //echo $i;	    
	 echo "<font color='white'><h3>subject:  {$row["subject_code"]}</h3></font><font color='yellow'><a href='form.php?subject_code=".$i."&reg=".$regulation_code."&stu=".$t."&semi=".$sem."&cycle=".$cy."' target='_blank'><h4>{$row["subject_name"]}</h4></a></font>";
